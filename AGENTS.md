...
| `.github/workflows/` | CI, the desktop packaging workflow, and the **proof** workflows — `e2e-linux.yml`, `e2e-runtime.yml`, `e2e-toolchain.yml`, `phase2-proofs.yml` and the six `daemon-*` files — every one of them `workflow_dispatch` only, because each renders a real video, assembles a real artefact or drives a real supervisor. `workflow_dispatch` registers from the **default branch**, so a new proof workflow is dispatchable only once it is on `main`, whatever `--ref` says | `actionlint` (`AC-4a`) |
