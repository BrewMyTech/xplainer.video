...
- e013f7a: `xplainer setup` offers to star the repository on GitHub, once, and only to a person.
...
/user/starred/...`, using the account it is already signed in as), then `GITHUB_TOKEN`/`GH_TOKEN` if
  ...
  **It asks nothing of someone who has already starred it.** Before anything is printed, `gh api
  /user/starred/...` is consulted: a `204` means the star is already there and the prompt is skipped
  in silence. That is better than the local record on its own, because `<state>/star.json` only knows
  about answers given on _this_ machine — somebody who starred the repository from the web would
  otherwise be asked again on every new laptop. Only a definite yes counts; `gh` missing, signed out
  or offline all fall through and the question is asked as before.

  The default is `[y/N]`, so Enter finishes the install rather than publishing an endorsement nobody
  typed. Starring is a public action on the reader's own account, and the prompt is only worth having
  if a yes means somebody meant it.
  ...
  _or_ no — so it is asked at most once per machine. And it can never fail the command: a GitHub
